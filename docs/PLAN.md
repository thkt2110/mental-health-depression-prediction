...
| **4** | 18/4 → 21/4 | Evaluation + Hoàn thiện Report |
...
| Thành viên | Công việc |
|---|---|
| **Kiệt** | `02_preprocessing.ipynb`: xử lý missing values, encoding categorical, merge conditional columns + **công thức chuẩn hoá (Z-score / Min-Max bằng LaTeX)** + viết Report phần Preprocessing |
| **Đàm Đạt** | `03_feature_engineering.ipynb`: Target Encoding (City, Profession, Degree) + **công thức Target Encoding bằng LaTeX**, interaction features + viết Report phần FE |
...
**Mục tiêu**: Train đa dạng models, tìm ra best model để phân tích sâu

> **Lưu ý**: Chỉ cần **XGBoost + LightGBM** ở advanced (+ Random Forest từ baseline để so sánh). Đã cắt CatBoost và Ensemble. Ưu tiên **Markdown phân tích sâu** và **công thức LaTeX** hơn là nhiều model.

| Thành viên | Công việc |
|---|---|
| **Tuấn** | `04_model_baseline.ipynb`: Logistic Regression + Decision Tree (5-Fold CV) + công thức sigmoid, Gini/Entropy |
| **Kiệt** | `04_model_baseline.ipynb`: Random Forest + SVM (5-Fold CV) + công thức Bagging, margin maximization + viết Report phần Model Development (Baseline) |
| **Đàm Đạt** | `05_model_advanced.ipynb`: XGBoost (tuning cơ bản, manual) + công thức loss function + regularization |
| **Tâm** | `05_model_advanced.ipynb`: LightGBM (tuning cơ bản, manual) + công thức Gradient Boosting update |
| **Thành** | Bảng so sánh RF vs XGBoost vs LightGBM + viết Report phần Model Development (Advanced) |

**Deliverable**: `04_model_baseline.ipynb` + `05_model_advanced.ipynb` hoàn chỉnh, biết được best model trong 3 models (RF, XGBoost, LightGBM)

---

## 🔶 GĐ4 — Evaluation & Hoàn Thiện Report (18/4 → 21/4)

**Mục tiêu**: Đánh giá tổng thể models + hoàn thiện nội dung Report

> **⭐ Trọng tâm**: SHAP Analysis là **mỏ vàng điểm số** của GĐ4. Phân tích mô hình hiểu gì về dữ liệu sẽ được đánh giá cao hơn nhiều so với một mô hình phức tạp nhưng “hộp đen”.

| Thành viên | Công việc |
|---|---|
| **Đàm Đạt** | `06_evaluation.ipynb`: Confusion Matrix, Classification Report + **SHAP Analysis (BẮT BUỘC)**: summary plot, bar plot, waterfall/force plot |
| **Tuấn** | `06_evaluation.ipynb`: Feature Importance so sánh 3 GBDT models, ROC-AUC curves, bảng so sánh tất cả models |
| **Thành** | Review toàn bộ notebooks, viết Report phần Experiments & Results |
...
**Deliverable**: `06_evaluation.ipynb` hoàn chỉnh, Report gần xong
...
| **Đàm Đạt** | Review nội dung technical trong Report |
...
| 4 | **Đàm Đạt** | Feature Engineering + Advanced Models + SHAP Analysis | 2-3 phút |
| 5 | **Tâm** | Results + Conclusion | 2 phút |
| 6 | **Tâm** | 🎬 **DEMO trực tiếp** (bắt buộc theo yêu cầu đề) | 1-2 phút |

> **Nội dung phần Demo** (Tâm thực hiện): Mở Jupyter Notebook trực tiếp trên màn hình, chạy hàm `predict()` với 1-2 bộ dữ liệu giả định (mock data), in ra kết quả: người này có bị trầm cảm không và xác suất bao nhiêu. Không cần fancy, chỉ cần rõ ràng.
...
| `05_model_advanced.ipynb` | Đàm Đạt + Tâm | 19/4 |
...
| Experiments & Results | Kiệt + Thành |
...
- [ ] Report PDF đủ 9 sections + YouTube link
- [ ] Video YouTube: cả 5 TV trình bày, giọng thật
- [ ] `TeamName.zip` nộp đúng hạn (< 20MB hoặc Google Drive)
