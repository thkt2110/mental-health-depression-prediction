...
Trong quá trình khám phá dữ liệu, nhóm tác giả phát hiện hai cột chứa dữ liệu bẩn nghiêm trọng do lỗi nhập liệu (\textit{data entry error}) hoặc hoán đổi cột (\textit{column swap}):
...
Do cơ chế MNAR được phát hiện ở Mục~\ref{subsec:missing_values}, việc điền missing bằng mean/median là \textbf{không phù hợp} --- vì giá trị missing mang ý nghĩa thực sự (sinh viên không có Work Pressure, người đi làm không có Academic Pressure). Nhóm tác giả áp dụng chiến lược gộp cột (\textit{merge}) như sau:
...
\textbf{Standardization} (Z-score normalization) --- biến đổi dữ liệu về phân phối chuẩn với trung bình $\mu = 0$ và độ lệch chuẩn $\sigma = 1$:
...
Trong dự án này, nhóm tác giả sử dụng \textbf{Standardization} (Phương trình~\ref{eq:standardization}) cho Logistic Regression và SVM vì phương pháp này ít bị ảnh hưởng bởi outliers hơn Min-Max.
